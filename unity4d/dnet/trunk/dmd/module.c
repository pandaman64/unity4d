...
Package::Package(Identifier *ident)
	: ScopeDsymbol(ident), homonym(NULL)
{
}
...
Dsymbol* Package::search(Loc loc, Identifier *ident, int flags)
{
    ...
    if (!s && homonym)
    {
        s = homonym->search(loc, ident, flags);
    }
    return s;
}
